...
-- =============================================================================
-- This script populates all tables with sample data for demonstration
-- Run after executing DDL scripts (01, 02, 03)
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- BRONZE LAYER - DIMENSION TABLES
-- -----------------------------------------------------------------------------

-- Suppliers (Multi-tier with geographic diversity)
INSERT INTO ${catalog}.${schema}.bronze_dim_supplier VALUES
  ...
  -- Tier 3 Suppliers (Hidden dependencies)
...
-- -----------------------------------------------------------------------------
-- BRONZE LAYER - FACT TABLES (Relationships)
-- -----------------------------------------------------------------------------
...
-- Material-Product Relationships (BOM)
...
-- Supplier-Supplier Tier Relationships
...
-- -----------------------------------------------------------------------------
-- SILVER LAYER - GRAPH NODES (Unified)
-- -----------------------------------------------------------------------------
...
SELECT
  ...
  latitude,
  longitude,
  -- Graph metrics (to be computed by pipeline)
  null as degree_centrality,
  null as betweenness_centrality,
  null as pagerank,
  -- Visualization
  CASE tier WHEN 1 THEN 50 WHEN 2 THEN 45 WHEN 3 THEN 40 ELSE 55 END as node_size,
...
-- -----------------------------------------------------------------------------
-- SILVER LAYER - GRAPH EDGES (Unified)
-- -----------------------------------------------------------------------------
...
SELECT
  ...
  GREATEST(1, LEAST(8, supply_share_pct / 12.5)) as edge_width,
  CASE
...
SELECT
  ...
  GREATEST(1, LEAST(6, quantity_per_unit)) as edge_width,
...
SELECT
  ...
  false as is_bottleneck,
  GREATEST(2, LEAST(10, annual_demand / 2500)) as edge_width,
...
SELECT
  ...
  supply_share_pct as utilization_pct,
  false as is_bottleneck,
  GREATEST(1, LEAST(6, supply_share_pct / 20)) as edge_width,
...
-- -----------------------------------------------------------------------------
-- GOLD LAYER - SUPPLY CHAIN HEALTH (Daily Snapshot)
-- -----------------------------------------------------------------------------

INSERT INTO ${catalog}.${schema}.gold_supply_chain_health VALUES (
  current_date(),
  -- Network stats
  14, 4, 5, 4, 1, 10, 5, 5, 52, 12, 15, 10, 15,
  -- Risk summary
  4, 3, 4, 3, 2,
  -- Flow metrics
  5420.0, 3, 78.5,
