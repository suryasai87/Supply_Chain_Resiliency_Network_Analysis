#!/usr/bin/env python3
"""
Build script for Supply Chain Resiliency Network Analysis with Neo4j
Builds frontend and packages for Databricks deployment as supply_chain_neo4j app
"""
...
def main():
    print("=" * 60)
    print("Building Supply Chain Neo4j App")
    print("=" * 60)

    project_root = Path(__file__).parent
    frontend_dir = project_root / "src" / "frontend"
    backend_dir = project_root / "src" / "backend"
    build_dir = project_root / "build-neo4j"
    ...
    # Skip frontend build if dist already exists (reuse from previous build)
    print("\n[2/5] Checking React frontend build...")
    frontend_dist = frontend_dir / "dist"
    if frontend_dist.exists():
        print("  Frontend already built, reusing existing build")
    else:
    ...
    # Copy app-neo4j.yaml as app.yaml
    print("\n[5/5] Copying app-neo4j.yaml as app.yaml...")
    app_yaml_src = project_root / "app-neo4j.yaml"
    app_yaml_dst = app_dir / "app.yaml"

    if app_yaml_src.exists():
        shutil.copy2(app_yaml_src, app_yaml_dst)
        print(f"  Copied app-neo4j.yaml to app/app.yaml")
    else:
        print("  ERROR: app-neo4j.yaml not found in repository root!")
        sys.exit(1)

    # Print summary
    print("\n" + "=" * 60)
    print("Build Summary - Supply Chain Neo4j")
    print("=" * 60)
    ...
    print("\n  Next step: Deploy using Databricks Apps CLI")
    print("=" * 60)
